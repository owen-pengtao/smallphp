...
class form{
  ...
  function get_validate($key) {
    ...
    }elseif (strstr($key, 'required,')){
      $arr = explode(',', $key);
      $arr_v = array();
      foreach($arr AS $v){
        $arr_v[] = $this->validate[$v];
      }
      $str = '{'.join(',', $arr_v).'}';
    }else{
   ...
   * @author owen 2008-6-12
   */
  function password($arr_type, $value='', $is_label=true) {
    //$arr_type[1] = 'password';

    $str = $this->_label($this->att_id($arr_type, 'password'), $is_label ? '输入密码' : '');
   ...
   * @author owen 2008-6-12
   */
  function password_re($arr_type, $value='', $is_label=true) {
    //$arr_type[1] = 'password_re';

    $str = $this->_label($this->att_id($arr_type, 'password'), $is_label ? '重复密码' : '');
    $str.= $this->_input($arr_type, 'password');
    $str.= ' value="'.$value.'"'.$this->__input();
    $str.= $this->__label();
    return $str;
   ...
   * @param array $arr_o array(cols宽, rows高, maxlength, readonly)
  ...
  function textarea($arr_type, $value='', $arr_o=array(50, 8), $str_tmp='') {
    $str_o = $arr_o[2] ? ' maxlength="'.$arr_o[2].'" ' : '';
    $str_o.= $arr_o[3]==true ? ' readonly="readonly" ' : '';
