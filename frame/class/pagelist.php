...
class pagelist{
  ...
  function __construct($db){
    ...
    $this->_set_table();
    $page = $_GET['page'] ? $_GET['page'] : $_GET['page_number'];
    $tmp_page = abs(intval(trim($page)));
  ...
  function get_rows_sql($sql_query, $row_count=0) {
    ...
    $arr["page_begin"] = (($this->page-1) * $this->page_size)+1;
    $arr["page_end"] = ($this->page>=$this->total_pages)?($arr["sum"]):($this->page*20);
    return $arr;
  }
  function show_page(){
  ...
  private function _set_total_records(){
    if ($this->total_records==0 or !isset($this->total_records)){
      if (empty($this->count_sql) and !empty($this->table["tablename"])){
        ...
        $sql = preg_replace("/SELECT(.*?)FROM(.*)(ORDER|LIMIT)(.*)/i", "SELECT count(*) AS count_id FROM\${2}", $this->sql);
      }
