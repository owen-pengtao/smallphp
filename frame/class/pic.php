...
class pic{
  ...
  private function is_equal(){
    $bool = false;
    if ($this->file_size[1] && $this->thumb_height && floor($this->file_size[0]/$this->file_size[1])==floor($this->thumb_width/$this->thumb_height)){
      $bool = true;
    }
    return $bool;
  }
  ...
  private function set_output_name($output_name){
    $this->create_dir(dirname($output_name));
    $this->file_extension = "jpg";
    if ($this->file_size[0]>=$this->thumb_width and $this->file_size[1]>=$this->thumb_height){
      ...
      $this->error_str = "源图片尺寸太小, 必须大于 ".$this->thumb_width." X ".$this->thumb_height." 。";
      $bool = false;
    }
    return $bool;
  }
